...
<body class="contPadding">
	...
	<div class="search">
		<form class="layui-form"> 
			...
			<div class="layui-inline">
				<input class="layui-input" name="discountType" id="discountType" autocomplete="off" placeholder="请输入折扣类型">
			</div>
	...
	layui.use(['table','form'], function(){
	  ...
	  table.render({
	    elem: '#tableId'
	    ,url:'${ctx}/productDiscount/toListData'
	    ,cellMinWidth: 80
	    ,page: true
	    ,cols: [[
	      {type:'checkbox'}
	      ,{field:'discountId', title: 'ID', sort: true}
	      ,{field:'discountSum', title: '满减件数'} 
	      ,{field:'discountName', title: '折扣名称'}
	      ,{field:'discountRemark', title: '折扣系数'}
	      ,{field:'discountType', title: '折扣类型'}
	  ...
	  table.on('tool(tableFilter)', function(obj){
	    ...
	    if(layEvent === 'detail'){
	      doDetails('折扣详情',data.discountId)
	    }
	    if(layEvent === 'del'){
	      doDelete('您确定要删除数据吗？删除后将无法恢复。',data.discountId);	
	    }
	    if(layEvent === 'edit'){
	      doForm('修改折扣',data.discountId);
	    }
	  });
	}); 
	...
	function doDelete(title,id){
		layer.confirm(title, function(index) {
			$.ajax({
			     url: '${ctx}/productDiscount/redelete',
			     type:"post",
			     //async:false,
			     dataType:"json",
			     data:{id:id},
	...
	function doForm(title,id){
		layer.open({
			  ...
			  maxmin: true,
			  area: ['100%','100%'],
			  content: '${ctx}/productDiscount/toForm?discountId='+id,
			  btn: ['立即提交'],
