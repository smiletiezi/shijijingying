...
<body class="contPadding">
	...
	layui.use(['table','form'], function(){
	  ...
	  table.render({
	    elem: '#tableId'
	    ,url:'${ctx}/productType/toListData'
	    ,cellMinWidth: 80
	    ,page: true
	    ,cols: [[
	      {type:'checkbox'}
	      ,{field:'productTypeId', title: 'ID', sort: true}
	      ,{field:'name', title: '产品类型名称'} 
	      ,{field:'typeCode', title: '父级ID'}
	  ...
	  table.on('tool(tableFilter)', function(obj){
	    ...
	    if(layEvent === 'del'){
	      doDelete('您确定要删除数据吗？删除后将无法恢复。',data.productTypeId);	
	    }
	    if(layEvent === 'edit'){
	      doForm('修改产品',data.productTypeId);
	    }
	  });
	}); 
	...
	function doForm(title,id){
		layer.open({
			  ...
			  maxmin: true,
			  area: ['100%','100%'],
			  content: '${ctx}/productType/toForm?productTypeId='+id,
			  btn: ['立即提交'],
